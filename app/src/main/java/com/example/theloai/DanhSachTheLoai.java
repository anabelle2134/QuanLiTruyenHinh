...
public class DanhSachTheLoai extends AppCompatActivity {
    ...
    private TheLoaiAdapter adapter; // lưu trữ biến cục bộ adapter
     ...
     */
    private ArrayList<TheLoai> loadData() {
        ArrayList<TheLoai> danhSachCongTrinh = TheLoaiDAO.layDanhSachTheLoai(database.getReadableDatabase());
        return danhSachCongTrinh;
    }

    private void setEvent() {
        // set adapter lên listview
        TheLoaiAdapter adapter = new TheLoaiAdapter(this, R.layout.cong_trinh_custom_listview, data);
        ...
        danhSachCongTrinhLv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                Toast.makeText(DanhSachTheLoai.this, "Bạn chọn " + theLoai.getTenTheLoai(), Toast.LENGTH_SHORT).show();
            }
        });
    }
                                ...
                                public void onClick(DialogInterface dialog, int which) {
                                    ...
                                    if (tenCongTrinhcantim.isEmpty() == false) {
                                        ...
                                        for (int i = 0; i < data.size(); i++) {
                                            if (data.get(i).getTenTheLoai().toUpperCase().equals(tenCongTrinhcantim) == false) {
                                                System.out.println(tenCongTrinhcantim);
                                                System.out.println(data.get(i).getTenTheLoai());
                                                data.remove(i);//remove những đối tượng không thoả tìm kiếm
                                                i--;
                                                adapter.notifyDataSetChanged(); // thôg báo thay đổi dữ liệu
                                                Toast.makeText(DanhSachTheLoai.this, "Tìm kiếm thành công !", Toast.LENGTH_SHORT).show();
                                            }
                                        }
                            ...
                            new DialogInterface.OnClickListener() {
                                ...
                                public void onClick(DialogInterface dialog, int which) {
                                    ...
                                    if (ten.isEmpty() || dc.isEmpty()) {
                                        Toast.makeText(DanhSachTheLoai.this, "Nhập đầy đủ thông tin để tạo mới!", Toast.LENGTH_SHORT).show();
                                        dialog.cancel();
                                    } else {
                                        TheLoai temp = new TheLoai(ten, dc);
                                        if (TheLoaiDAO.themTheLoai(temp, db) == 0) { // thêm vào csdl thành công
